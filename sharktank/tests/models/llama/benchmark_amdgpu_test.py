...
class BenchmarkLlama3_1_70B(BaseBenchmarkTest):
    ...
    def testBenchmark70B_fp8_TP8_Decomposed(self):
        output_file_name = self.dir_path_70b / "fp8_decomposed_tp8"
        output_mlir = self.llama70b_fp8_decomposed_artifacts_tp8.create_file(
            suffix=".mlir", prefix=output_file_name
        )
        output_json = self.llama70b_fp8_decomposed_artifacts_tp8.create_file(
            suffix=".json", prefix=output_file_name
        )
        output_vmfb = self.llama70b_fp8_decomposed_artifacts_tp8.create_file(
            suffix=".vmfb", prefix=output_file_name
        )
        output_shard_file_name = (
            self.artifacts_dir
        ...
        if output_shard_file_name.exists():
            self.llama70b_fp8_decomposed_artifacts_tp8.irpa_path = (
                output_shard_file_name
            )
        export_return_code = self.llama70b_fp8_decomposed_artifacts_tp8.export_to_mlir(
            mlir_path=output_mlir,
            json_path=output_json,
        )
        self.llama70b_fp8_decomposed_artifacts_tp8.compile_to_vmfb(
            mlir_path=str(output_mlir),
            vmfb_path=output_vmfb,
            hal_dump_path=output_file_name,
            cwd=self.repo_root,
        ...
        # benchmark prefill
        self.llama70b_fp8_decomposed_artifacts_tp8.iree_benchmark_vmfb(
            hip_device_id=self.iree_device,
            vmfb_name=output_vmfb,
            irpa_path=self.irpa_path_fp8,
...
class BenchmarkLlama3_1_405B(BaseBenchmarkTest):
    def setUp(self):
        ...
        self.iree_run_prefill_nondecomposed_args_2048_f16_tp8 = (
            [
                "--function=prefill_bs4",
                f"--input=@{self.prefill_args_bs4_2048_f16_tp8}/tokens.npy",
                f"--input=@{self.prefill_args_bs4_2048_f16_tp8}/seq_lens.npy",
